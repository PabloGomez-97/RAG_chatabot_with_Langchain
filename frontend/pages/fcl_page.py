...
from backend.fcl.config import (
    OPENAI_API_KEY, WELCOME_MESSAGE, 
    LOCAL_VECTOR_STORE_DIR, TMP_DIR,
    extract_ports_from_query, detect_maritime_fcl_query_type,
    get_port_region, analyze_maritime_route_direction,
    extract_container_type_from_query
)
from backend.fcl.core import (
...
class FCLPageService:
    """Servicio para la página de FCL marítimo - maneja la lógica de UI"""
    
    def __init__(self):
        pass
    
    def get_maritime_fcl_response(self, prompt):
        ...
        try:
            with st.spinner("🔍 Buscando tarifas de contenedores FCL..."):
                ...
                with st.chat_message("assistant"):
                    ...
                    self.display_maritime_fcl_metrics(validation, port_info, response.get("source_documents", []), container_type)
                    
                    # Análisis detallado de fuentes
                    with st.expander("📋 **Análisis Detallado de Rutas FCL**"):
                        self.display_fcl_route_analysis(response.get("source_documents", []), port_info)
    ...
    def display_maritime_fcl_metrics(self, validation: dict, port_info: dict, source_docs: list, container_type: str):
    ...
    def display_fcl_route_analysis(self, sources: list, port_info: dict):
    ...
    def enhanced_sidebar_maritime_fcl(self):
        ...
        with tab1:
            ...
            with col1:
                if st.button("🔨 Crear Sistema FCL", type="primary"):
                    self.create_maritime_fcl_system()
            with col2:
        ...
        with tab2:
            ...
            if st.button("📖 Cargar Base de Datos", type="primary"):
                self.load_existing_maritime_fcl_vectorstore()

    def create_maritime_fcl_system(self):
    ...
    def load_existing_maritime_fcl_vectorstore(self):
    ...
    def render_chat_interface(self):
        ...
        if hasattr(st.session_state, 'ejemplo_selected'):
            ...
            if hasattr(st.session_state, 'chain'):
                self.get_maritime_fcl_response(prompt)
            else:
...
def render_fcl_page():
    """Función principal para renderizar la página de FCL marítimo"""
    service = FCLPageService()
    
    # Sidebar
    service.enhanced_sidebar_maritime_fcl()
